# 从第一性原理思考 Agentic Engineering
 ...
 > 导语：AI 让写代码变快了，但快不等于对——在复杂系统中，AI 生成的代码往往"能跑"却未必"可靠"。如果你正在用 AI Agent 开发生产级系统，并且发现"能跑"和"能上线"之间的鸿沟越来越大——本文为你而写。我们从第一性原理出发，推导出一套系统性的 Agentic Engineering 方法论，并将其落地为一个开箱即用的开源框架。
...
0. [为什么用第一性原理？](#0-为什么用第一性原理)
1. [问题与机遇](#1-问题与机遇)
2. [目标](#2-目标)
3. [第一性原理推导](#3-第一性原理推导)
4. [基于 Skill 的 Agentic Engineering 框架](#4-基于-skill-的-agentic-engineering-框架)
...
- **实现层**的缺陷：命名不符合项目约定、并发模型使用不当、内存管理不安全——这些是复杂系统中隐蔽缺陷的主要来源
...
## 4. 基于 Skill 的 Agentic Engineering 框架
...
| 模块 | 职责 | 触发方式 |
|------|------|---------|
| **Workflow** | SDLC 全链条工作流（需求→设计→编码→测试→评审→验证门禁） | 直接触发 |
...
此外，工作流末端引入了**客观验证门禁**（`workflow-verification`）：它把实践 4 中「行为层 / 系统层 / 实现层静态规范」的验证落地为可执行检查，并通过**基线对比**只追究本次改动**新增**的违规。这使「完成」的判定从 Agent 的主观汇报，变为脚本可裁决的客观结果——`code-generation` 在代码评审通过后须过此门禁才能交付。门禁是可选的、对存量项目无侵入（无配置则跳过），并以语言中立的脚本承载，不绑定具体 Agent 平台。
...
本章描述的是框架的**架构和设计原则**，而非具体实现。我们已将上述方法论落地为一个开源框架：

> **[agentic-engineering-framework](https://git.woa.com/tdsql3.0/agentic-engineering-framework)** — 基于 Skill 的模块化 Agentic Engineering 框架，包含完整的 SDLC Workflow、Best Practices、Self-Refinement 机制及项目定制指南。
...
1. **Addy Osmani:** ["Agentic Engineering"](https://addyosmani.com/blog/agentic-engineering/), 2025.
2. **GitHub Copilot Productivity Study:** ["Quantifying GitHub Copilot's impact on developer productivity and happiness"](https://github.blog/news-insights/research/research-quantifying-github-copilots-impact-on-developer-productivity-and-happiness/), 2022.
3. **Anthropic Internal Study:** ["How AI is transforming work at Anthropic"](https://www.anthropic.com/research/how-ai-is-transforming-work-at-anthropic), 2025.
4. **Nelson F. Liu, Kevin Lin, John Hewitt, et al.:** ["Lost in the Middle: How Language Models Use Long Contexts"](https://arxiv.org/abs/2307.03172), Stanford University & University of Washington, 2023.
...
6. **用乔哈里窗分析人机协作的灵感来源：** [《用"好问题"让AI有效输出》](https://km.woa.com/articles/show/637725?kmref=user_articles_list).