...
确认 `openspec/changes/<ticket>-<change-name>/proposal.md` 存在。不存在则停止：
...
- 路径：`openspec/changes/<ticket>-<change-name>/ui-mockup-A.html`。
- Tailwind CDN，可直接打开。
- 轻量档只生成 1 个方案。
...
随后写入 `openspec/changes/<ticket>-<change-name>/ui-spec.md`，**每个 section 必须填实，不得只留空标题**：
...
```text
UI 规格已产出：openspec/changes/<ticket>-<change-name>/ui-spec.md
