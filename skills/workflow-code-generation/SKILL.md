...
8. **交付门（机器判定）**：Fast-Path 免传 spec / tasks，但必须传 Run 级 Review 报告与长期知识影响结论：命中时运行 `python <本 skill 目录>/scripts/check_delivery.py --review-report <review-report.json> --knowledge-impact hit`；未命中时运行 `python <本 skill 目录>/scripts/check_delivery.py --review-report <review-report.json> --knowledge-impact none --knowledge-impact-reason "<具体理由>"`。门禁同时校验工作区干净；非 0 → 补齐 Review 报告、知识影响结论或提交后重跑。用户要求不提交的改动是工作区干净检查的唯一豁免，仍须在报告中标注知识影响结论。
