...
- 验收标准：
    ...
    - [ ] 按 proposal §6.3 合同一落实逐任务 Review 粒度：Production 下每 Task 按 `review_profile` 触发对应档位 Review，禁止统一收尾；核对与 §3.4 的一致性。
    - [ ] 按 proposal §6.3 合同二落实 Plan 总门：保留为进入执行前的独立总守卫，不拆散到各转移。
