...
- 状态: 已核对（2026-07-27）。已知需核对：`framework-unification.md` §19.3 放弃「把两个 Skill 目录直接合并」。交付时须确认本 Change 的退役未被实现成变相的目录合并——退役编排与合并目录是不同动作，区别必须在文档中写明。

## 实际 Diff 核对

- 已核对（2026-07-27）。须包含 `git diff --stat`，并单独确认归档中的 `opsx-*` 引用未改动。

---

### 任务 0：[x] 确认 change 2043 与 2044 已交付
...
- 验收标准：
    - [x] 确认 2043 交付：执行链已统一，治理语义由叠加门承载，三判据与强度判据均通过。
    - [x] 确认 2044 交付：写状态路径已统一。
    - [x] 缺任何一个，停止本 Change，记录阻塞，不提前退役。
    - [x] 确认 `opsx-*` 当前已无活跃任务执行依赖（2043 已让任务不再走它）。
...
- verification: `python -m pytest scripts -q`
- 验收标准：
    - [x] 六个入口逐一拆解为「执行编排职责」与「方法论职责」。
    - [x] 全仓库检索 `opsx-`，列出 Skill、Command、文档、脚本中的全部引用，标注活跃／归档。
    - [x] 归档引用单独标注为不改动。
    - [x] 确认编排职责已被叠加门承载，无残留依赖。
    - [x] 记录每个入口方法论内容的具体清单（不是只给分类）。
...
- verification: `python -m pytest scripts -q`
- 验收标准：
    - [x] 按三条标准逐项判定：已被承载→删除；仍有独立价值→迁移；只适用双链结构→删除。
    - [x] 每项判定附理由，无「默认全删」或「默认全留」。
    - [x] 迁移目标位置按内容性质逐项确定并记录，不预设统一目标。
    - [x] 迁移的内容保留原文，不改写为「适配新结构」（改写属另一动作）。
    - [x] 迁移后无方法论内容只存在于 `opsx-*` 中而未判定去向。

### 任务 3：[x] 安装器白名单与无活跃残留核验
...
- 验收标准：
    - [x] 白名单移除 `opsx-*` 条目。
    - [x] Tooling 安装验证不受影响。
    - [x] Production 安装指向叠加门结构，验证可用。
    - [x] `--switch-profile` 切换逻辑验证正确。
    - [x] 活跃引用（Skill、Command、文档、脚本）中 `opsx-*` 执行编排引用清零。
    - [x] 归档引用未改动。

### 任务 4：[x] 文件处置决策门（需老板确认）
...
- verification: `python -m pytest scripts -q`
- 验收标准：
    - [x] 向老板提交处置方案：删除活跃 `opsx-*` 入口，还是保留为归档。
    - [x] **明确删除不可逆**，列出将删除的具体文件清单。
    - [x] 得到老板明确确认后才执行；未确认前不动任何文件。
    - [x] 若保留为归档，记录归档位置与方式。
    - [x] 执行后复核：无残留引用、安装器仍正确。
...
- 验收标准：
    - [x] `framework-unification.md` 记录 `opsx-*` 执行编排已退役，Production 语义由叠加门承载。
    - [x] 记录退役与 §19.3「目录合并」的区别，明确本 Change 未做合并。
    - [x] `install-agentic-framework/overview.md` 更新白名单。
    - [x] 记录方法论内容的迁移去向汇总。
