...
Production 校验器是只读生命周期裁决器；Tooling 控制器是有状态执行引擎。二者的合并按三层分别评估（change 2036 引入），不作整体裁决。
...
### 8.5 「一条执行链」的可判定定义（change 2036）

**判据一：状态源唯一。** 任一时刻，一个 Task 的状态字段只有一个写入者。§3.3 已规定「校验器只读，不维护平行状态」「`workflow_control.py` 负责校验和原子写回，不另建平行状态」——判据一是这两条在收敛后的延续，不是新增要求。核验方式：静态查找对 `- 状态:` 与任务头标记的写入点。

**判据二：门为叠加。** Profile 门只增加状态转移的前置条件，不新增、不删除、不重定向状态转移本身。即 `production` 能让一个转移**不发生**，但不能让它转向 `tooling` 下不存在的目标状态。核验方式：转移图基线比对。

**判据三：降级等价（主判据）。** 把 `governance_profile` 从 `production` 降到 `tooling` 后，对同一输入的执行结果必须与纯 Tooling 实现逐字节相同。判据一是必要但不充分条件（两套逻辑各自只读一半状态也能满足）；判据二最贴近「叠加而非替换」的直觉，但要求先有转移图基线；判据三可机械核验，且顺带覆盖 §19.1 第二条理由——分支膨胀到无法降级归一时判据自动失败。

**失败后果**：判据无法执行（例如降级路径不存在）与判据不通过，同等处理，均判定为越界，必须回退到合并前形态。「无法执行」不得按「暂不适用」放行。

**判据三的已知局限**：只能证明「关掉 production 后等于 tooling」，不能证明「打开 production 后治理强度不低于现状」；后者由 §3.2 条件 3 承担，两者必须同时满足，缺一不可。判据三的可执行实现已落地（change 2042）：`scripts/downgrade_equivalence.py` 对同一输入跑「production 降级」与「纯 Tooling」两条命令路径，归一为「退出码＋排序后错误编号集合」逐字节比对，给出通过／不通过／无法执行三态（「无法执行」与「不通过」同判越界但原因分开标注，报告头部固定写明局限）；降级开关由 change 2041 的 `--governance-profile` 提供。当前两轨输出格式不可比（Tooling lint 无错误编号体系），默认路径对如实报告「无法执行」，门层合并后以合并引擎的两路径调用。
