...
Review 同时输出 Markdown 和 Run 级 JSON。机器门校验 `verdict`、P0/P1 数量、`scope`、`review_profile` 和轮次，但 Reviewer 的语义判断仍需独立 Judge 负责（`skills/workflow-code-review/SKILL.md:14-59,270-300`）。Review 报告可为 Envelope（裁决字段在 `payload` 内）或旧式扁平 JSON（裁决字段在顶层），机器门按同一套字段校验；Envelope 额外要求顶层 `artifact_type` 为 `review-report`。
