...
- verification:
  - [ ] `python -m py_compile skills/workflow-code-generation/scripts/workflow_control.py`
  - [ ] `python -m pytest scripts/test_workflow_control.py -q` 全部通过
  - [ ] 新增测试覆盖：缺 `--verify-report`、文件不存在、JSON 非法、`verdict != "PASS"` 四类失败路径均返回非 0 且 `tasks.md` 未被写入
  - [ ] 新增测试覆盖：`verdict == "PASS"` 时行为与改动前一致（不影响既有正常路径）
- artifacts:
  - `skills/workflow-code-generation/scripts/workflow_control.py`
  - `scripts/test_workflow_control.py`
- 子任务:
  - [ ] 1.1: `_build_arg_parser` 新增 `--verify-report` 参数
  - [ ] 1.2: 实现 `_validate_verify_report` 纯函数
  - [ ] 1.3: 在 `quality_passed` 分支接入校验，CLI 层完成文件读取与错误提示
  - [ ] 1.4: 调用 `workflow-test-generation` 为新增校验路径生成测试（覆盖正常路径 + 4 类失败路径）
  - [ ] 1.5: 运行测试，全部通过
...
- verification:
  - [ ] `python skills/workflow-code-generation/scripts/lint_skill_graph.py`（或仓库现有等价命令）无新增错误/警告
  - [ ] 人工核对：新增说明的字段名、取值范围与 spec.md 2.4 节完全一致
  - [ ] 人工核对：`verdict`/`p0_count`/`p1_count` 与现有 Markdown 报告的「总体结论」「正式问题」区可相互推导，不引入第二套判断标准
- artifacts:
  - `skills/workflow-code-review/SKILL.md`
- 子任务:
  - [ ] 2.1: 在 Step 7 后新增「机器可读产物」章节，给出 JSON 示例
...
- verification:
  - [ ] `python -m py_compile skills/workflow-code-generation/scripts/check_delivery.py`
  - [ ] `python -m pytest scripts/test_check_delivery.py -q` 全部通过
  - [ ] 新增测试覆盖：缺 `--review-report`、文件不存在、JSON 非法、`verdict != "PASS"`、`p0_count>0`、`p1_count>0` 六类失败路径均返回非 0
  - [ ] 新增测试覆盖：`verdict=="PASS"` 且 P0/P1 均为 0 时，行为与改动前一致
- artifacts:
  - `skills/workflow-code-generation/scripts/check_delivery.py`
  - `scripts/test_check_delivery.py`
- 子任务:
  - [ ] 3.1: 实现 `check_review_report` 函数
  - [ ] 3.2: `main()` 接入 `--review-report` 参数与校验
  - [ ] 3.3: 调用 `workflow-test-generation` 生成测试（正常路径 + 六类失败路径）
  - [ ] 3.4: 运行测试，全部通过

### 任务 4: [ ] validate_change.py 的 OPSX038/OPSX032 加 Review Report 字段校验
- 状态: 未开始
...
- verification:
  - [ ] `python -m py_compile scripts/validate_change.py`
  - [ ] `python -m pytest scripts/tests/test_validate_change.py -q` 全部通过
  - [ ] 新增测试覆盖：Task 级与 change 级各自的「缺字段」「文件不存在」「JSON 非法」「verdict 不是 PASS」「P0/P1>0」五类失败路径均产出对应 finding
  - [ ] 新增测试覆盖：字段合法且文件满足条件时，不产出新增 finding（不破坏现有通过路径）
- artifacts:
  - `scripts/validate_change.py`
  - `scripts/tests/test_validate_change.py`
- 子任务:
  - [ ] 4.1: 新增 `TASK_REVIEW_REPORT_RE` 正则与 `_validate_review_report` 共用校验函数
  - [ ] 4.2: 接入 `_validate_tasks`（Task 级，OPSX038）
  - [ ] 4.3: 接入 `_validate_delivery`（change 级，OPSX032）
  - [ ] 4.4: 调用 `workflow-test-generation` 生成测试（两处校验点 × 五类失败路径 + 正常路径）
  - [ ] 4.5: 运行测试，全部通过

### 任务 5: [ ] 同步调用方文档与现状描述
- 状态: 未开始
- 文件: `skills/workflow-code-generation/reference/delegated-execution-guide.md`（修改）, `skills/opsx-code-generation/SKILL.md`（修改，如涉及）, `docs/framework-features-status-and-comparison.md`（修改）
...
- verification:
  - [ ] `python skills/workflow-code-generation/scripts/lint_skill_graph.py` 无新增错误/警告
  - [ ] 人工核对：三份文档中出现的参数名/字段名与 Task 1/2/3/4 的最终实现逐一核对一致
- artifacts:
...
- 子任务:
  - [ ] 5.1: 更新 `delegated-execution-guide.md`
  - [ ] 5.2: 检查并更新 `opsx-code-generation/SKILL.md`（如需要）
  - [ ] 5.3: 更新 `framework-features-status-and-comparison.md`
