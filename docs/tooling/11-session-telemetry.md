...
1. **积累账本**：持续用 `--history .agentic-framework/metrics/session-history.jsonl` 收数据。「复审轮新增 P0/P1 比例」「需人工率」两个指标（ADR 003 的质量验证口）的解析已实现（2026-07：报告模板加轮次字段与「（新增）」标记，脚本按流分组重建循环）；指标要有意义还需账本里积累足够多带轮次标记的新会话。2026-07 首次遵从率验证识别出 3 条新轨迹，均带明确轮次；账本中的 4 份「?」来自旧格式报告，因此暂不加固模板措辞。
